...
document.querySelectorAll(".quiz-page .directory-card").forEach((card) => {
...
function renderStateStage(card, stage) {
  ...
  if ((stage === 4 && !isTerritoryPage) || (stage === 5 && isTerritoryPage)) {
    ...
    if (isTerritoryPage) {
      ...
      slot.innerHTML = '<span class="state-facts"><span>Motto: ' + escapeHtml(stateMotto) + '</span><span>Area: ' + escapeHtml(area) + ' sq mi</span><span>2020 population: ' + escapeHtml(population) + '</span><span>National Parks: ' + nationalParks + '</span><span>National Monuments: ' + nationalMonuments + '</span></span>';
    }
  }
  ...
  if (stage === 5 && !isTerritoryPage) {
    ...
    slot.innerHTML = '<span class="cycle-label">All together</span><ul class="state-bullets"><li><strong>Capital:</strong> ' + escapeHtml(capital) + '</li><li><strong>Motto:</strong> ' + escapeHtml(stateMotto) + '</li><li><strong>Map:</strong> state location view</li><li><strong>Flag:</strong> state flag view</li><li><strong>Area:</strong> ' + escapeHtml(area) + ' sq mi</li><li><strong>2020 population:</strong> ' + escapeHtml(population) + '</li><li><strong>National Parks:</strong> ' + nationalParks + '</li><li><strong>National Monuments:</strong> ' + nationalMonuments + '</li></ul>';
  }
...
document.querySelectorAll(".state-cycle-card").forEach((card, cardIndex) => {
  if (!document.body.classList.contains("territory-page")) {
    card.dataset.cardColor = String(cardIndex % 5);
  }
  const surface = card.querySelector(".state-cycle-surface");
  ...
  if (surface) {
    surface.addEventListener("click", () => {
      ...
      const finalStage = document.body.classList.contains("territory-page") ? 6 : 4;
      const next = current >= finalStage ? 1 : current + 1;
...
if (stateSortBy && stateSortDirection && stateCycleGrid) {
  ...
  function updateStateDirectionLabels(resetToAscending = false) {
    const selectedDirection = resetToAscending ? "asc" : stateSortDirection.value;
    const labels = directionLabels[stateSortBy.value];
    stateSortDirection.replaceChildren(
    ...
    stateSortDirection.value = selectedDirection;
  }

  function sortStateCards() {
    const category = stateSortBy.value;
    const direction = stateSortDirection.value === "desc" ? -1 : 1;
    const orderedCards = [...stateCards].sort((a, b) => {
      if (category === "name") {
  ...
  stateSortDirection.addEventListener("change", sortStateCards);
